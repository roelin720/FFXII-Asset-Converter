...
#define _(type) || (ext == type)
		...
		if (0 _("ah")_("txt")_("config")_("cfg")_("json")_("ini")_("xml")_("yaml")_("h")_("hpp")_("c")_("cpp")_("cs")_("java"))
		{
...
#undef check
		...
		if (bool& processing = file_processing_map.insert({ folder_str, false }).first->second; !processing)
		{
			...
			GUI::thread_pool.push([&processing, filter, folder_str, sort_specs, &arc_rec, map_key](int)
			{
				...
				for (VBFArchive::TreeNode* child : node->children)
				{
					...
					{
						info.type_str = PhyreIO::GetExtension(info.name);
						if (info.type_str != "dae.phyre" && info.type_str != "dds.phyre" &&
							info.type_str != "win.mab" && info.type_str != "win.sab")
						{
							info.type_str = PhyreIO::GetLastExtension(info.type_str);
						}
	...
	std::string right_aligned_elipsis_str(const std::vector<std::string>& folder, float max_len)
	{
		...
		std::string str = PhyreIO::Join(folder);
		...
		{
			str = PhyreIO::Join(std::vector<std::string>(it, folder.end()));
			if (ImGui::CalcTextSize(str.c_str()).x <= max_len)
			{
				return "..." + str;
			}
...
Phyre::FileBrowser::FileBrowser(const std::string& id) :
	id(id)
{}

bool Phyre::FileBrowser::init()
{
	free();
...
bool Phyre::FileBrowser::set_current_path(const std::string& path)
{
	try
	{
		...
		set_current_folder(PhyreIO::Segment(parent_path));
...
Phyre::FileBrowser::DialogCode Phyre::FileBrowser::run_dialog()
{
	draw_content();
...
void Phyre::FileBrowser::draw_popup_messages()
{
...
void Phyre::FileBrowser::popup_error(const std::string& msg)
{
...
bool Phyre::FileBrowser::valid(const std::string& path)
{
	std::string disk_path = path, arc_file_path;
	if (path.find(".vbf") != std::string::npos)
	{
		if (!VBFUtils::Separate(path, disk_path, arc_file_path))
		{
			return false;
		}
	}
	if (std::filesystem::exists(disk_path) == false)
	{
		return false;
	}
	if (arc_file_path.size())
	{
		auto find = archives.find(disk_path);
		...
		if (const VBFArchive::TreeNode* node = find->second.archive.find_node(arc_file_path); !node)
		{
			return false;
		}
	}
...
bool Phyre::FileBrowser::valid(const std::vector<std::string>& path)
{
	return valid(PhyreIO::Join(path));
}

void Phyre::FileBrowser::draw_content()
{
	if (!valid(PhyreIO::Join(cur_parent_folder)))
	{
	...
	ImGui::BeginChild(id.c_str(), ImVec2(0, 0), true, ImGuiDockNodeFlags_PassthruCentralNode | ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoDocking);

	draw_archive_modal_window();
	draw_nav_bar();

	ImGuiDockNodeFlags dockspaceFlags = ImGuiDockNodeFlags_None;
	ImGuiID dockspaceID = ImGui::GetID((id + "dockspace").c_str());
...
void Phyre::FileBrowser::draw_archive_modal_window()
{
...
void Phyre::FileBrowser::draw_current_folder_pane()
{
	ImGui::Begin("current_folder_pane", nullptr, 0);

	std::string cur_folder_str = PhyreIO::Join(cur_parent_folder);
	...
	{
		ImGui::TableSetupColumn("Name", 0, 0.0f, FileInfo::FieldID::Name);
		ImGui::TableSetupColumn("Type", 0, 0.0f, FileInfo::FieldID::Type);
		ImGui::TableSetupColumn("Size", 0, 0.0f, FileInfo::FieldID::Size);
		if (!in_vbf)
		{
		...
		if (in_vbf)
		{
			std::string disk_path, arc_path;
			if (VBFUtils::Separate(cur_folder_str, disk_path, arc_path))
			{
				auto find = archives.find(disk_path);
				if (find != archives.end())
				{
					children = get_child_files(arc_path, filter, is_new, sort_specs, find->second);
				}
			}
		}
		else
		...
		else
		{
			for (int i = 0; i < children.size(); ++i)
			{
				...
				GUITexture& icon =
					...
					children[i].typeID == FileInfo::Image ? image_icon :
					children[i].typeID == FileInfo::Model ? model_icon :
					children[i].typeID == FileInfo::Music ? music_icon :
					children[i].typeID == FileInfo::Text ? text_icon :
					children[i].typeID == FileInfo::Video ? video_icon :
					children[i].typeID == FileInfo::Exe ? exe_icon :
					children[i].typeID == FileInfo::Dll ? dll_icon :
					file_icon;
				ImVec4 icon_tint =
					children[i].typeID == FileInfo::Folder ? ImVec4(1.0f, 0.92f, 0.36f, 1.0f) :
					children[i].typeID == FileInfo::Archive ? ImVec4(1.0f, 0.9f, 0.16f, 1.0f) :
					children[i].typeID == FileInfo::VBFArchive ? ImVec4(0.61f, 1.0f, 0.46f, 1.0f) :
					children[i].typeID == FileInfo::Image ? ImVec4(0.55f, 1.0f, 0.88f, 1.0f) :
					children[i].typeID == FileInfo::Model ? ImVec4(0.55f, 1.0f, 0.88f, 1.0f) :
					children[i].typeID == FileInfo::Music ? ImVec4(0.55f, 1.0f, 0.88f, 1.0f) :
					children[i].typeID == FileInfo::Text ? ImVec4(0.79f, 0.91f, 1.0f, 1.0f) :
					children[i].typeID == FileInfo::Video ? ImVec4(0.55f, 1.0f, 0.88f, 1.0f) :
					children[i].typeID == FileInfo::Exe ? ImVec4(0.66f, 0.54f, 1.0f, 1.0f) :
					children[i].typeID == FileInfo::Dll ? ImVec4(0.79f, 0.72f, 1.0f, 1.0f) :
					ImVec4(1.0f, 1.0f, 1.0f, 1.0f);

				ImGui::Image(icon, ImVec2(icon.width, icon.height), ImVec2(0, 0), ImVec2(1, 1), icon_tint);
				ImGui::SameLine();
					...
					{ 
						if (confirm_overwrite)
						{
							int msgboxID = MessageBoxA(
								NULL, "Overwrite existing file?", "Overwrite confirmation",
				...
				if ((!in_vbf || children[i].typeID & FileInfo::Folder) && ImGui::BeginPopupContextItem(("cmenu_" + PhyreIO::Join(cur_parent_folder) + children[i].name).c_str()))
				{
					if (children[i].typeID & (FileInfo::Folder | FileInfo::VBFArchive))
					{
						std::vector<std::string> new_folder = cur_parent_folder;
						...
						{
							if (ImGui::Selectable("Open Folder in Explorer")) open_natively(PhyreIO::Join(new_folder));
						}
					}
					else if(!in_vbf)
					{
						if (ImGui::Selectable("Open Location in Explorer")) open_natively(PhyreIO::Join(cur_parent_folder));
					}
...
void Phyre::FileBrowser::draw_folder_tree_pane()
{
...
ImRect Phyre::FileBrowser::draw_folder_tree_node(const std::vector<std::string>& path, bool& clicked, bool selected, bool leaf, const std::function<void()> pre_children, generator child_generator, const std::function<ImRect(void*)>& child_operator, int type_code)
{
	switch (type_code)
	{
	...
	bool open = ImGui::TreeNodeEx(("##" + PhyreIO::Join(path)).c_str(), ImGuiTreeNodeFlags_SpanAvailWidth | (leaf ? ImGuiTreeNodeFlags_Leaf : 0));
	ImGui::PopStyleColor();
...
ImRect Phyre::FileBrowser::draw_vbf_tree(const std::vector<std::string>& vbf_path, const std::string& vbf_path_str, const std::string& vbf_filename)
{
...
ImRect Phyre::FileBrowser::draw_vbf_tree(const std::vector<std::string>& vbf_path, const std::vector<std::string>& cur_arc_name, const std::vector<std::string>& arc_foldername, const VBFArchive::TreeNode& node)
{
	...
	{
		if (ImGui::BeginPopupContextItem((PhyreIO::Join(vbf_path) + PhyreIO::Join(arc_foldername)).c_str()))
		{
...
void Phyre::FileBrowser::draw_quick_access_pane()
{
	...
	else
	{
		...
		for (size_t i = 0; i < pinned_folders.size(); ++i)
		{
			...
			ImGui::PopID();
			ImGui::SameLine();
			if (ImGui::Button((right_aligned_elipsis_str(PhyreIO::Segment(pinned_folders[i]), ImGui::GetContentRegionAvail().x) + "##pinned_" + std::to_string(i)).c_str()))
			{
				if (valid(pinned_folders[i]))
				{
					set_current_folder(PhyreIO::Segment(pinned_folders[i]));
				}
				else
				{
...
void Phyre::FileBrowser::draw_nav_bar()
{
	...
	if (ImGui::BeginMenuBar())
	{
		...
		if (text_edit_mode)
		{
			...
			{
				std::string cur_parent_folder_str = PhyreIO::Join(cur_parent_folder);
				std::strncpy(text_buffer, cur_parent_folder_str.c_str(), std::min(sizeof(text_buffer), strlen(cur_parent_folder_str.c_str()) + 1));
			...
			if (ImGui::InputText("##cur_parent_folder_text_edit", text_buffer, sizeof(text_buffer), ImGuiInputTextFlags_EnterReturnsTrue) ||
				...
				std::string cur_parent_path_str = PhyreIO::Normalise(text_buffer);
		...
		else
		{
			...
			for (; min_seg >= 0; --min_seg)
			{
				...
				if (min_seg < cur_parent_folder.size() - 1 && address_unused_rect.Min.x + next_seg_space >= address_unused_rect.Max.x - unused_space_min)
				{
					break;
				}
...
bool Phyre::FileBrowser::draw_footer()
{
	...
	std::strncpy(file_name_buffer, cur_filename.c_str(), cur_filename.size());
	...
	if (!filter_names.empty())
	{
		...
		static int selected_filter = filter_index;
		ImGui::SetNextItemWidth(filters_size);
		if (ImGui::Combo("##filter_combo", &selected_filter, filter_names.data(), filter_names.size()))
		{
	...
	{
		bool is_valid = valid(PhyreIO::Join(cur_parent_folder) + "/" + cur_filename);

		if (!is_valid && !cur_parent_folder.empty() && cur_parent_folder.back() == cur_filename)
		{
			cur_parent_folder.pop_back();
			is_valid = valid(PhyreIO::Join(cur_parent_folder) + "/" + cur_filename);
		}

		bool parent_valid = valid(PhyreIO::Join(cur_parent_folder));
		...
		{
			int msgboxID = MessageBoxA(
				NULL, "Overwrite existing file(s)?", "Overwrite confirmation",
				MB_ICONWARNING | MB_YESNO
			);
...
void Phyre::FileBrowser::load_pinned_folders()
{
...
void Phyre::FileBrowser::push_pinned_folder(const std::vector<std::string>& folder)
{
	pinned_folders.push_back(PhyreIO::Join(folder));
	save_pinned_folders();
}

void Phyre::FileBrowser::erase_pinned_folder(size_t index)
{
	pinned_folders.erase(pinned_folders.begin() + index);
	save_pinned_folders();
}

void Phyre::FileBrowser::save_pinned_folders()
{
	try
	{
	...
	{
		std::cerr << "Failed to save pinned folders";
	}
}

void Phyre::FileBrowser::set_current_folder(const std::vector<std::string>& folder)
{
	...
	std::string folder_str = std::string(PhyreIO::Join(folder));
...
void Phyre::FileBrowser::set_current_folder_to_prev()
{
...
void Phyre::FileBrowser::set_current_folder_to_next()
{
	if (!folder_history.empty())
	{
